...
public class Util_Check_News_Parameter {
	...
	public static Map<String, String[]> checkNewsMap(Map<String, String[]> map, Map<String,String> errorMsgs) {
		map = checkNewsNews_no(map, errorMsgs);
		map = checkNewsNewstype_no(map, errorMsgs);
		return map;
	}
	public static Map<String, String[]> checkNewsNews_no(Map<String, String[]> map, Map<String,String> errorMsgs) {
		...
		if(map.containsKey(key_news_no)) {
			...
			if(news_no == null || news_no.trim().length()==0) {
				...
				errorMsgs.put("news_no","查無此編號---消息編號，格式不符，如:N001");
				map.remove(key_news_no);
			}
		}
		return map;
	}
	public static Map<String, String[]> checkNewsNewstype_no(Map<String, String[]> map, Map<String,String> errorMsgs) {
		...
		if(map.containsKey(key_newstype_no)) {
			...
			if(newstype_no == null || newstype_no.trim().length()==0) {
				...
				errorMsgs.put("newstype_no","查無此編號---消息編號，格式不符，如:NT001");
