...
public class VenueJNDIDAO implements VenueDAO_interface{
	...
	private static final String INSERT =
			"INSERT INTO venue (v_no ,v_name ,vt_no ,reg_no ,"
			+ "v_lat ,v_long ,v_pic, v_address ,"
			+ "v_phoneno ,v_status ,v_func) "
			+ "VALUES ('V'||LPAD(to_char(VENUE_SEQ.NEXTVAL), 6, '0'),?,?,?,?,?,?,?,?,?,?)";
	...
	private static final String UPDATE = 
			"UPDATE venue SET v_name=?, vt_no=?, reg_no=?, "
			+ "v_lat=?, v_long=?, v_pic=?, v_address=?, "
			+ "v_phoneno=?, v_status=?, v_func=? where v_no=?";
	...
	public void update(VenueVO venueVO) {
		...
		try {
			...
			pstmt.setString(9, venueVO.getV_status());
			pstmt.setString(10, venueVO.getV_func());
			pstmt.setString(11, venueVO.getV_no());
